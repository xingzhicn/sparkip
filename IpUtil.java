...
public class IpUtil {
    ...
    /**
     * binary search ip attribution
     * @param lines
     * @param ip
     * @return
     */
    public static String binarySearch(List<String[]> lines, Long ip) {
        int low = 0;
        int high = lines.size() - 1;
        while (low <= high) {
            int middle = (low + high) / 2;
            if ((ip >= Long.parseLong((lines.get(middle))[0])) && (ip <= Long.parseLong((lines.get(middle)[1])))) {
                try{
                    return lines.get(middle)[2];
                }catch (Exception e){
                    e.printStackTrace();
                    return "none";
                }
            }
            if (ip <  Long.parseLong(lines.get(middle)[1])) {
                high = middle - 1;
            } else {
                low = middle + 1;
            }
